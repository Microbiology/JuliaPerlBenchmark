...
for itr in 100 1000 10000 50000 100000 250000 472897; do
	{ time perl calculate_fasta_median_length.pl uniprot-bacteriophage-2.fasta; } 2> ./tmpPerlTime${itr}.log
	grep [a-zA-Z0-9] ./tmpPerlTime${itr}.log | awk -v var=$itr '{ print $0"\tPerl\t"var }' | sed 's/.m\(.\)./\1/' | sed 's/\([0-9]\)s/\1/' > ./tmpPerlTime${itr}form.log
	{ time julia calculate_fasta_median_length.jl uniprot-bacteriophage-2.fasta; } 2>> ./tmpJuliaTime${itr}.log
